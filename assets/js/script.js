...
let calcIt = () => {
    ...
    if (nb_1 && nb_2) {
        ...
        switch (calc) {
            case Infinity:
                result.innerHTML = `<span class="${calc > 0 ? 'success' : 'danger'}" >${0 + '%'}</span>`;
                break;
            default:
                result.innerHTML = `<span class="${calc > 0 ? 'success' : 'danger'}" >${calc.toFixed(2) + '%'}</span>`;
                break;
        }
    }
